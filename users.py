...
def read_all():
    ...
    serialized_data = user_schema.dump(users).data
...
def create(user):
    ...
    if existing_user is None:                                                   # Can we insert this user?
        ...
        new_user = schema.load(user, session = sql_alchemy_db.session).data
        sql_alchemy_db.session.add(new_user)                                    # Add the user to the database
        sql_alchemy_db.session.commit()
        serialized_data = schema.dump(new_user).data                            # Serialize and return the newly created user in the response
...
def update(user_id, user):
    ...
    if update_user is None:                                                     # Are we trying to find a user that does not exist?
        ...
        update = schema.load(user, session = sql_alchemy_db.session).data
        ...
        sql_alchemy_db.session.commit()
        serialized_data = schema.dump(update_user).data                         # return updated user in the response
